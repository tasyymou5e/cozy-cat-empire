...
/** Scans recently played cloud saves for data-integrity problems. */
export async function runSyncHealthCheck(
  ...
  for (const save of (saves || []) as Array<{ user_id: string; game_state: unknown }>) {
    const userId = save.user_id;
    const gameState = save.game_state as JobGameState | null;
    ...
    if (cats.length === 0 && gameState.day > 1) {
      issues.push({
        ...
        description: `Day ${gameState.day} with 0 cats`,
        severity: 'critical',
      });
      savesWithIssues.add(userId);
    }
  ...
  const issueSummary = {
    ...
    by_type: issues.reduce<Record<string, number>>((acc, issue) => {
  ...
  console.log(
    `Sync health check complete: ${result.saves_checked} saves, ${result.total_issues} issues, ${executionTime}ms`,
  );
