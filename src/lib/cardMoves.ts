...
const PERSONALITY_MOVES: Record<CatPersonality, { name: string; desc: string; baseDmg: number }> = {
  ...
  curious: { name: 'Investigate', desc: 'Scouts the field. If the rival has a special trait, this attack does 20 more damage.', baseDmg: 20 },
  lazy: { name: 'Nap Attack', desc: 'This cat falls asleep mid-battle. Heal 30 HP and gain +10 resistance next turn.', baseDmg: 10 },
  shy: { name: 'Shadow Fade', desc: 'This cat hides completely, avoiding all damage during the rival\'s next turn.', baseDmg: 20 },
};
...
const TRICK_MOVES: Record<TrickId, { name: string; desc: string; baseDmg: number }> = {
  ...
  rollOver: { name: 'Rolling Fury', desc: 'Rolls into all nearby rivals, dealing 20 damage to each resting cat.', baseDmg: 50 },
  jump: { name: 'Aerial Leap', desc: 'If this cat has no damage, this attack does 40 more damage.', baseDmg: 60 },
  fetch: { name: 'Supreme Fetch', desc: 'Retrieves a hidden treat that restores focus and boosts the next move.', baseDmg: 80 },
};

const BREED_SPECIALS: Record<CatBreed, { name: string; desc: string; baseDmg: number }> = {
  persian: { name: 'Psychic Purr', desc: 'A hypnotic purr that reveals the rival\'s strategy. Discard 1 of their boosts.', baseDmg: 50 },
  bengal: { name: 'Royal Roar', desc: 'Uses 2 stamina for a thunderous roar. The defending cat is now Stunned.', baseDmg: 80 },
  tabby: { name: 'Street Smarts', desc: 'If this cat is the underdog, this attack does 30 more damage.', baseDmg: 40 },
  ragdoll: { name: 'Starlight Cuddle', desc: 'Heal 20 HP from this cat. If at full HP, this does 50 more damage.', baseDmg: 100 },
  siamese: { name: 'Frost Whisper', desc: 'The defending cat is now Frozen. On a miss, costs 1 stamina.', baseDmg: 60 },
  'maine-coon': { name: 'Titan Tackle', desc: 'A devastating charge that also does 20 damage to itself.', baseDmg: 90 },
  'british-shorthair': { name: 'Iron Guard', desc: 'This cat takes 30 less damage during the rival\'s next turn.', baseDmg: 50 },
  stray: { name: 'Alley Rush', desc: 'Launches a flurry of scrappy swipes. Each hit does 20 more damage.', baseDmg: 30 },
};
