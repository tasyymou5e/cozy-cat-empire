...
const ResourcePanel = lazy(() =>
  import('../ResourcePanel').then((m) => ({ default: m.ResourcePanel }))
);
      ...
      <TabsContent value="chores" className="mt-0 panel-animate-in">
        <PanelErrorBoundary panelName="ChorePanel">
          <Suspense fallback={<PanelSkeleton rows={4} />}>
            <ChorePanel
              onDoChore={(choreId, baseReward) =>
                dispatchAction(GameActions.DO_CHORE, { choreId, baseReward })
              }
            />
          </Suspense>
        </PanelErrorBoundary>
        ...
        <PanelErrorBoundary panelName="ResourcePanel">
          <Suspense fallback={<PanelSkeleton rows={4} />}>
            <ResourcePanel
              ...
              money={money}
              catCount={catCount}
              onBuyResource={(resource, cost) =>
                dispatchAction(GameActions.BUY_RESOURCE, {
