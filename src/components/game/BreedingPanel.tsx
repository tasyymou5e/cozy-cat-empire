...
export function BreedingPanel({ 
  ...
  const getRelationshipBetween = (cat1Id: string, cat2Id: string) => {
    ...
    return relationships.find(
      r => (r.catId1 === cat1Id && r.catId2 === cat2Id) || 
           (r.catId1 === cat2Id && r.catId2 === cat1Id)
    );
  };
  ...
  const getRelationshipBadgeVariant = (level: string) => {
    switch (level) {
      case 'bestFriend': return 'default';
      case 'friend': return 'secondary';
      case 'neutral': return 'outline';
      case 'rival': return 'destructive';
                    ...
                    >
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <div className="flex items-center gap-1">
                          <CatVisual cat={pair.cat1} size="xs" equippedCostumeId={catCostumes?.[pair.cat1.id]} />
                          <span className="text-xs truncate max-w-[60px]">{pair.cat1.name}</span>
                        </div>
                        <span className="text-muted-foreground">+</span>
                        <div className="flex items-center gap-1">
                          <CatVisual cat={pair.cat2} size="xs" equippedCostumeId={catCostumes?.[pair.cat2.id]} />
                          <span className="text-xs truncate max-w-[60px]">{pair.cat2.name}</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <Badge variant={getRelationshipBadgeVariant(pair.relationshipLevel)} className="text-xs">
                          {getRelationshipEmoji(pair.relationshipLevel as any)}
                          {pair.bonus > 0 ? `+${pair.bonus}%` : pair.bonus < 0 ? `${pair.bonus}%` : '0%'}
                        </Badge>
                        <Button 
                          variant="outline" 
                          size="sm" 
