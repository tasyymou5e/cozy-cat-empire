        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge className={effectBadge[event.effect]}>
              {event.effect === 'positive' ? '✨ Good' : event.effect === 'negative' ? '⚠️ Bad' : 'ℹ️ Event'}
            </Badge>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-2 text-xs">
            {event.moneyChange && event.moneyChange !== 0 && (
              <span className={event.moneyChange > 0 ? 'text-green-600' : 'text-red-600'}>
                💰 {event.moneyChange > 0 ? '+' : ''}{event.moneyChange}
              </span>
            )}
            {event.resourceChange && Object.entries(event.resourceChange).map(([key, val]) => (
              val !== 0 && (
            ...
            {event.catEffect?.healthChange && (
              <span className={event.catEffect.healthChange > 0 ? 'text-green-600' : 'text-red-600'}>
                ❤️ {event.catEffect.healthChange > 0 ? '+' : ''}{event.catEffect.healthChange}
              </span>
            )}
            {event.catEffect?.happinessChange && (
              <span className={event.catEffect.happinessChange > 0 ? 'text-green-600' : 'text-red-600'}>
                😊 {event.catEffect.happinessChange > 0 ? '+' : ''}{event.catEffect.happinessChange}
              </span>
            )}
            {event.reputationChange && (
              <span className="text-purple-600">
                ⭐ +{event.reputationChange} rep
              </span>
            )}
          </div>
        </div>
