...
const ACTIVITIES = [
  ...
  },
  {
    ...
    name: 'Group Nap',
    emoji: '😴',
    cost: { toys: 0, treats: 0 },
    happinessBonus: 5,
    relationshipBonus: 3,
  },
];

export function GroupActivitiesPanel({
  cats,
  groups,
  treats,
  toys,
          ...
          {friendlyGroups.length === 0 ? (
            ...
            <div className="space-y-4">
              {friendlyGroups.map((group) => {
                const memberCats = group.memberIds
                ...
                return (
                  <div
                    key={group.id}
                    className="p-3 rounded-lg border border-green-200 bg-green-50/50"
                  >
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {ACTIVITIES.map((activity) => (
                        <Button
                          ...
                          onClick={() =>
                            onGroupActivity(group.id, activity.id as 'play' | 'treat' | 'nap')
                          }
                          className="flex flex-col h-auto py-2 text-xs"
                        >
                          <span className="text-lg">{activity.emoji}</span>
                          <span className="text-xs">{activity.name.split(' ')[1]}</span>
                          {activity.cost.toys > 0 && (
                            <span className="text-xs text-muted-foreground">
                              🎾{activity.cost.toys}
                            </span>
                          )}
                          {activity.cost.treats > 0 && (
                            <span className="text-xs text-muted-foreground">
                              🍬{activity.cost.treats}
                            </span>
                          )}
                        </Button>
                      ))}
