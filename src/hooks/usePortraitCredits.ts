...
// Default package config (can be overridden by game_config)
const DEFAULT_PACKAGE_CONFIG: PortraitPackageConfig = {
  cost: 5000,
  portraits: 3,
};
...
export function usePortraitCredits(): UsePortraitCreditsReturn {
  ...
  const fetchCredits = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('manage-portrait-credits', {
        method: 'GET',
      });

      if (error) {
        console.error('Error fetching portrait credits:', error);
        setCredits({ creditsRemaining: 0, totalPurchased: 0, totalUsed: 0, lastPurchaseAt: null });
      } else {
        setCredits(data);
      }
    } catch (err) {
      console.error('Failed to fetch portrait credits:', err);
      setCredits({ creditsRemaining: 0, totalPurchased: 0, totalUsed: 0, lastPurchaseAt: null });
    } finally {
      setIsLoading(false);
    }
  ...
  const fetchPackageConfig = useCallback(async () => {
    try {
      ...
      if (!error && data?.value) {
        ...
        setPackageConfig({
          cost: config.cost || DEFAULT_PACKAGE_CONFIG.cost,
          portraits: config.portraits || DEFAULT_PACKAGE_CONFIG.portraits,
        });
      }
    } catch (err) {
      console.error('Failed to fetch package config:', err);
    }
  }, []);
  ...
  const purchaseCredits = useCallback(async (): Promise<{ success: boolean; newMoneyBalance?: number }> => {
    if (!user) {
      toast({
        ...
        description: 'Please login to purchase portrait credits.',
        variant: 'destructive',
      });
    ...
    try {
      ...
      if (error) {
        ...
        toast({
          ...
          description: error.message || 'Failed to purchase portrait credits.',
          variant: 'destructive',
        });
        return { success: false };
      }
      ...
      toast({
        ...
        description: `You now have ${data.creditsRemaining} portrait credits.`,
      });
