...
interface UseCollectionProgressReturn {
  ...
  breedProgress: { collected: number; total: number; items: { id: string; name: string; emoji: string; collected: boolean }[] };
  personalityProgress: { collected: number; total: number; items: { id: string; name: string; emoji: string; collected: boolean }[] };
  costumeProgress: { collected: number; total: number; items: { id: string; name: string; emoji: string; collected: boolean }[] };
  trickProgress: { collected: number; total: number; items: { id: string; name: string; emoji: string; collected: boolean }[] };
...
export function useCollectionProgress(
  ...
  const overallProgress = useMemo(() => {
    const totalCollected = breedProgress.collected + personalityProgress.collected + 
                          costumeProgress.collected + trickProgress.collected;
    const totalItems = breedProgress.total + personalityProgress.total + 
                      costumeProgress.total + trickProgress.total;
