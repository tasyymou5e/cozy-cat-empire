...
import {
  ActiveCoopChallenge, CoopChallengeInvite, CoopChallenge, CoopChallengeType,
  COOP_CHALLENGE_TEMPLATES, isCoopChallengeCompleted,
...
export function useCoopChallenges(
    ...
    const loadFromCloud = async () => {
      ...
      try {
        const { data: challengeData, error: challengeError } = await supabase
          .from('coop_challenges').select('*')
          .or(`initiator_id.eq.${userId},partner_id.eq.${userId}`)
          .in('status', ['active', 'completed']);

        if (challengeError) {
          log.error('Error loading coop challenges:', challengeError);
          ...
          const challenges: ActiveCoopChallenge[] = challengeData
            ...
            .map((c) => {
              ...
              return {
                ...
                partnerAvatar: friend?.avatar_emoji || '😺', initiatorId: c.initiator_id,
                ...
                startedAt: c.started_at ?? new Date().toISOString(), expiresAt: c.expires_at,
        ...
        const { data: pendingData, error: pendingError } = await supabase
          .from('coop_challenge_invites').select('*')
          .eq('recipient_id', userId).eq('status', 'pending');

        if (pendingError) {
          log.error('Error loading pending invites:', pendingError);
        } else if (pendingData) {
          const now = new Date();
          setPendingInvites(pendingData.filter((i) => new Date(i.expires_at) > now).map((i) => {
            ...
            return {
              ...
              sentAt: i.sent_at ?? new Date().toISOString(), expiresAt: i.expires_at,
            };
          }));
        }
        ...
        if (sentError) {
          log.error('Error loading sent invites:', sentError);
        } else if (sentData) {
          const now = new Date();
          setSentInvites(sentData.filter((i) => new Date(i.expires_at) > now).map((i) => ({
            id: i.id, challenge: i.challenge_data as unknown as CoopChallenge,
            senderId: i.sender_id, senderName: 'You', senderAvatar: '😺',
            sentAt: i.sent_at ?? new Date().toISOString(), expiresAt: i.expires_at,
          })));
        }
      } catch (e) {
        log.error('Failed to load coop challenges from cloud:', e);
      } finally {
        setLoading(false);
      }
    };
  ...
  const sendInvite = useCallback(async (friendId: string, challengeId: string): Promise<boolean> => {
    ...
    if (!friend) { toast({ title: 'Error', description: 'Friend not found', variant: 'destructive' }); return false; }
    const challenge = COOP_CHALLENGE_TEMPLATES.find((c) => c.id === challengeId);
    if (!challenge) { toast({ title: 'Error', description: 'Challenge not found', variant: 'destructive' }); return false; }
    const existingActive = activeChallenges.find((c) => c.partnerId === friendId && c.status === 'active');
    if (existingActive) { toast({ title: 'Already in Challenge', description: `You already have an active challenge with ${friend.display_name || 'this friend'}`, variant: 'destructive' }); return false; }

    try {
      const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
      const { data, error } = await supabase.from('coop_challenge_invites').insert({
        challenge_template_id: challengeId, challenge_data: JSON.parse(JSON.stringify(challenge)),
        sender_id: userId, recipient_id: friendId, expires_at: expiresAt,
      }).select().single();
      if (error) throw error;
      setSentInvites((prev) => [...prev, { id: data.id, challenge, senderId: userId, senderName: 'You', senderAvatar: '😺', sentAt: data.sent_at ?? new Date().toISOString(), expiresAt: data.expires_at }]);
      playSound?.('success');
      toast({ title: '🤝 Invite Sent!', description: `Challenge invite sent to ${friend.display_name || 'your friend'}` });
      return true;
    } catch (e) {
      log.error('Error sending coop invite:', e);
  ...
  const acceptInvite = useCallback(async (inviteId: string): Promise<boolean> => {
    ...
    try {
      ...
      const { data: challengeData, error: challengeError } = await supabase.from('coop_challenges').insert({
        challenge_template_id: invite.challenge.id, challenge_data: JSON.parse(JSON.stringify(invite.challenge)),
        initiator_id: invite.senderId, partner_id: userId, expires_at: expiresAt.toISOString(),
      }).select().single();
      if (challengeError) throw challengeError;
      await supabase.from('coop_challenge_invites').update({ status: 'accepted', responded_at: new Date().toISOString() }).eq('id', inviteId);
      setActiveChallenges((prev) => [...prev, {
        id: challengeData.id, challenge: invite.challenge, partnerId: invite.senderId,
        partnerName: invite.senderName, partnerAvatar: invite.senderAvatar, initiatorId: invite.senderId,
        myProgress: 0, partnerProgress: 0, status: 'active', startedAt: startDate.toISOString(),
        expiresAt: expiresAt.toISOString(), rewardClaimed: false,
      }]);
      setPendingInvites((prev) => prev.filter((i) => i.id !== inviteId));
      playSound?.('friendship');
      toast({ title: `${invite.challenge.emoji} Challenge Started!`, description: `You and ${invite.senderName} are now working on "${invite.challenge.name}"!` });
      return true;
    } catch (e) {
      log.error('Error accepting coop invite:', e);
  ...
  const declineInvite = useCallback(async (inviteId: string): Promise<boolean> => {
    try {
      await supabase.from('coop_challenge_invites').update({ status: 'declined', responded_at: new Date().toISOString() }).eq('id', inviteId);
      setPendingInvites((prev) => prev.filter((i) => i.id !== inviteId));
      toast({ title: 'Invite Declined', description: 'The challenge invite has been declined' });
      return true;
    } catch (e) {
      log.error('Error declining invite:', e);
      return false;
    }
  }, []);
  ...
  const cancelInvite = useCallback(async (inviteId: string): Promise<boolean> => {
    try {
      ...
      toast({ title: 'Invite Cancelled', description: 'The challenge invite has been cancelled' });
      return true;
    } catch (e) {
      log.error('Error cancelling invite:', e);
      return false;
    }
  }, []);

  const updateProgress = useCallback(async (challengeType: CoopChallengeType, increment: number = 1) => {
    if (!userId) return;
    const matchingChallenges = activeChallenges.filter((c) => c.status === 'active' && c.challenge.challengeType === challengeType);
    for (const challenge of matchingChallenges) {
      const newProgress = challenge.myProgress + increment;
      const isInitiator = challenge.initiatorId === userId;
      try {
        const updateData = isInitiator ? { initiator_progress: newProgress } : { partner_progress: newProgress };
        ...
        setActiveChallenges((prev) => prev.map((c) => {
          ...
          if (isCoopChallengeCompleted(updatedChallenge) && c.status === 'active') {
            supabase.from('coop_challenges').update({ status: 'completed' }).eq('id', challenge.id);
            playSound?.('levelUp');
            toast({ title: `${challenge.challenge.emoji} Coop Challenge Complete!`, description: `You and ${challenge.partnerName} completed "${challenge.challenge.name}"! Claim your reward!` });
            return { ...updatedChallenge, status: 'completed' as const };
          }
          return updatedChallenge;
        }));
      } catch (e) {
        log.error('Error updating coop progress:', e);
      }
    }
  ...
  const claimReward = useCallback(async (challengeId: string): Promise<{ coins: number; bonus: number } | null> => {
    ...
    if (!challenge || challenge.status !== 'completed' || challenge.rewardClaimed) return null;
    const baseReward = challenge.challenge.rewardCoins;
    const bonus = Math.floor(baseReward * (challenge.challenge.bonusMultiplier - 1));
    try {
      const isInitiator = challenge.initiatorId === userId;
      const updateData = isInitiator ? { initiator_reward_claimed: true } : { partner_reward_claimed: true };
      await supabase.from('coop_challenges').update(updateData).eq('id', challengeId);
      setActiveChallenges((prev) => prev.map((c) => (c.id === challengeId ? { ...c, rewardClaimed: true } : c)));
      playSound?.('coin');
      toast({ title: `${challenge.challenge.emoji} Reward Claimed!`, description: `You earned ${baseReward} coins + ${bonus} coop bonus = ${baseReward + bonus} coins!` });
      return { coins: baseReward, bonus };
    } catch (e) {
      log.error('Error claiming coop reward:', e);
      return null;
    }
  }, [userId, activeChallenges, playSound]);
