    ...
    (cat1Id: string, cat2Id: string) => {
      setState((prev) => {
        ...
        const kitten: Cat = {
          ...
          hunger: 70,
          value: Math.floor(
            (BREEDS[parent1.breed].baseValue + BREEDS[parent2.breed].baseValue) / 2 +
              Math.random() * 50 +
              compatibility.bonus
          ),
          age: 0,
        ...
        relationshipSystem.addEvent(
          ...
          parent2,
          'positive',
          `${parent1.name} and ${parent2.name} had a kitten together`,
          15,
          prev.day
        );
        ...
        if (isBestFriendBreed) {
          showMessage(
            `💕 Perfect Match! ${parent1.name} and ${parent2.name} (best friends) had a kitten: ${name}!`,
            'success'
          );
          ...
          showMessage(
            `🎉 ${parent1.name} and ${parent2.name} had a kitten: ${name}!${bonusMsg}`,
            'success'
          );
        }
        playSound?.('meow');
